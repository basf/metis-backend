...
def submit_calc(user_session):
    _, answer = make_request(bff_host + '/v0/calculations', {'dataId': 1}, 'POST', headers={'Cookie': user_session})
...
#thread = threading.Timer(2, submit_structure, args=[content, user_session])
#thread.start()
...
response = http.request('GET', bff_host + '/stream', preload_content=False, headers={'Accept': 'text/event-stream'})
